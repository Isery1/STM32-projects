"""
Startup checks: RC522 SPI (version reg), LAN/Wi‑Fi link state, then auth endpoint reachability.
Skips RFID when USE_MOCK_RFID is set. Skip entire sequence with SKIP_BOOT_CHECKS=1 or --no-boot-check.

Use on_step callbacks for GUI progress: on_step(phase, state, detail)
  phase: "hardware" | "network" | "server"
  state: "running" | "skip" | "ok" | "error"
"""
...
def _interface_states() -> Dict[str, Dict[str, str]]:
    """Best-effort link/operstate from /sys (Linux). Empty if unavailable."""
    base = "/sys/class/net"
...
def _any_connected(states: Dict[str, Dict[str, str]]) -> Tuple[bool, List[str]]:
    """True if at least one non-loopback iface looks up with carrier (if carrier file exists)."""
    notes: List[str] = []
    ok_any = False
