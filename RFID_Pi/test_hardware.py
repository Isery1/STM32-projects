"""
Interactive RC522 bring-up script for the Raspberry Pi.

Initializes SPI + GPIO, prints the chip version register (sanity for wiring), then optionally enters a
loop that waits for a physical tag. Not used in production startup — see :mod:`boot_sequence` for a
non-blocking version check only.
"""
...
def run_diagnostics():
    """
    Walk through version-register read, human-readable interpretation, and live tag polling.

    Cleans up GPIO in a ``finally`` block so the bus is released even after Ctrl+C or errors.
    """
    print("\n" + "="*50)
